...
#endif
//    uint64_t hs4_row_id = table_size - 4;
//    uint64_t hs5_row_id = table_size - 5;
//    uint64_t hs6_row_id = table_size - 6;
//    uint64_t hs7_row_id = table_size - 7;
//    uint64_t hs8_row_id = table_size - 8;
//    uint64_t hs9_row_id = table_size - 9;
//    uint64_t hs10_row_id = table_size - 10;
//    std::vector<uint64_t> vec_hots = {hs1_row_id, hs2_row_id, hs3_row_id,
//                                     hs4_row_id, hs5_row_id };
//    std::vector<uint64_t> vec_hot;
//    // 随机选择三个数据项
//    std::random_device rd;
//    std::mt19937 gen(rd());
//    std::uniform_int_distribution<> dis(0, vec_hots.size() - 1);
//
//    while (vec_hot.size() < 3) {
//        int index = dis(gen);
//        uint64_t selected_item = vec_hots[index];
//        // 检查是否已选择过该数据项
//        if (std::find(vec_hot.begin(), vec_hot.end(), selected_item) == vec_hot.end()) {
//            vec_hot.push_back(selected_item);
//        }
//    }
